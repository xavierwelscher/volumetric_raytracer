bool renderTerrain(vec3 pos, vec3 rd, float t, float step_size, inout float transmittance, inout vec3 color) {
				...
				if (pos.y < actualHeight) {
								...
								float atmosphereDensity = 0.015;
								float fogFactor = exp(-atmosphereDensity * t);
								vec3 horizonColor = vec3(0.65, 0.75, 0.85);
								tColor = mix(horizonColor, tColor, fogFactor);
